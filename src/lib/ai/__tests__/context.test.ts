...
describe("getContext()", () => {
  it("returns selected text with a sentence module when no chapterText", async () => {
    ...
    const result = await getContext(selectedText, null, modules, null);
  ...
  it("extracts the sentence containing selected text from chapterText for sentence module", async () => {
    ...
    const result = await getContext(selectedText, chapterText, modules, null);
  ...
  it("combines context from multiple enabled modules", async () => {
    const modules = [
      ...
      makeModule({ type: "sentence", id: "s2" }),
    ];

    const selectedText = "birds were singing";
    const result = await getContext(selectedText, null, modules, null);

    // Both sentence modules contribute the selected text
    ...
    expect(result.source).toBe("s1,s2");
  });
  ...
  it("returns empty context when no modules are enabled", async () => {
    ...
    const result = await getContext("test", null, modules, null);
  ...
  it("ignores disabled modules and only processes enabled ones", async () => {
    const modules = [
      ...
      makeModule({ type: "sentence", isEnabled: true, id: "enabled" }),
    ];
  ...
  it("queries aggregator for dictionary module", async () => {
    ...
    const result = await getContext("hello", null, modules, aggregator);
  ...
  it("caps dictionary context at 1500 characters", async () => {
    ...
    const result = await getContext("test", null, modules, aggregator);
  ...
  it("combines dictionary and sentence context", async () => {
    ...
    const result = await getContext("walked", "She walked.", modules, aggregator);
  ...
  it("returns a Promise from getContext", () => {
    const modules = [makeModule({ type: "sentence" })];
    const result = getContext("test", null, modules, null);
  ...
  it("uses offset to find correct sentence when text appears multiple times", async () => {
    ...
    const result = await getContext(selectedText, chapterText, modules, null, 25);
  ...
  it("falls back to first match when offset does not match any sentence", async () => {
    ...
    const result = await getContext(selectedText, chapterText, modules, null, 9999);
